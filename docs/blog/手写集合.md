...
// 写myCall目的就是，实现 js 中常用的 call 方法
Function.prototype.myCall = function(context){
    // 判断参数是否有, 如果没有参数 默认为 window
    var context = context || window; 
    // 将参数中挂载一个函数，使这个函数等于调用者(this ===  fn, 因为this是谁调用它，它就指向谁)
    context.fn = this;
    // 取传的参数，第一个之后的所有参数。 这个是传给调用函数用的
    const args = [...arguments].slice(1); 
    // 执行函数。此时这个函数的 this 指向了context。原因是 context = {name: '静静', fn: function}
    const result = context.fn(args); 
    // 删除这个 fn的原因是，fn 就根本不属于context 参数之中，我们只是用 o 改变 fn 中的 this
    delete context.fn; 
    return result
}
const o = {
...
function fn(arg){
    console.log('打印出来看 this 指向谁', this.name, arg[0]);
    // 打印出来看 this 指向谁 静静 {name: '振凯'}
...
Array.prototype.newMap2 = function(fn) {
...
Array.prototype.newReduce = function(fn, value){
    let prev;
    if(typeof value === 'undefind'){
        prev = this[0];
        this.slice(1).forEach((element, index, arr) => {
            prev = fn(prev, element, index, arr)
        })
    } else {
        prev = value;
...
Array.prototype.newReduce2 = function(fn,value){
    const array = this;
    let pre = value || array[0];
